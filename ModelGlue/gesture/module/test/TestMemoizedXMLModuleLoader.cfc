...
<cffunction name="testLoading" returntype="void" access="public">
	...
	<!--- Messages --->
	<cfset assertTrue(arrayLen(obj.messages) eq 2, "message not found or more than one message found") />
	...
	<!--- Results --->
	<cfset assertTrue(structCount(obj.results) eq 3, "results not found or two results not found") />
	...
	<cfset assertTrue(arrayLen(obj.results["CFNULLKEYWORKAROUND"]) eq 2, "implicit result result not  found or more than one result mapped") />
	...
	<!--- Views --->
	<cfset assertTrue(arrayLen(obj.views) eq 2, "view not found or more than one view found") />
