<cfcomponent output="false">
	...
	<!---
		...
		Shawn Gorrell's popular cf_xssblock tag (http://www.illumineti.com/documents/xssblock.txt).
	...
	<cfset variables.instance.sqlFilter = "select,insert,update,delete,drop,--,alter,xp_,information_schema,execute,sp_sqlexecute,table_cursor,cast\(,exec\(,declare">
	...
	<cffunction name="scan" output="false" access="public" returntype="Void">
		...
		<cfloop collection="#arguments.object#" item="item">
			...
			<cfset StructInsert(arguments.object,"#newitem#",contents,true)/>
		</cfloop>
		...
		<cfloop collection="#arguments.object#" item="item">
			<cfif ListContainsNoCase(exFF,item,',') eq false>
				<cfset temp = filterWords(arguments.object[item])/>
				<cfset itemname = REReplaceNoCase(item,"[^a-zA-Z0-9_]","","All")>
				<cfif temp.detected eq true><cfset detected = detected + 1/></cfif>
				<cfif arguments.objectname eq "cookie" and variables.instance.allowJSAccessCookies eq false>
					<cfheader name="Set-Cookie" value="#itemname#=#temp.cleanText#;HttpOnly">
		...
		<cfloop collection="#arguments.object#" item="item">
			<cfif ListContainsNoCase(exFF,item,',') eq false>
				<cfset temp = filterCRLF(arguments.object[item])/>
				<cfset itemname = REReplaceNoCase(item,"[^a-zA-Z0-9_]","","All")>
		...
		<cfloop collection="#arguments.object#" item="item">
			<cfif ListContainsNoCase(exFF,item,',') eq false>
				<cfset temp = filterSQL(arguments.object[item])/>
				<cfset itemname = REReplaceNoCase(item,"[^a-zA-Z0-9_]","","All")>
				<cfif temp.detected eq true><cfset detected = detected + 1/></cfif>
				<cfif arguments.objectname eq "cookie" and variables.instance.allowJSAccessCookies eq false>
					<cfheader name="Set-Cookie" value="#itemname#=#temp.cleanText#;HttpOnly">
		...
		<cfif variables.instance.escapeChars eq true>
			<cfloop collection="#arguments.object#" item="item">
			<cfif ListContainsNoCase(exFF,item,',') eq false>
				<cfif isNumeric(arguments.object[item]) eq false>
					<cfset itemname = REReplaceNoCase(item,"[^a-zA-Z0-9_]","","All")>
					<cfset temp = escapeChars(arguments.object[item])/>
					<cfset "#arguments.objectname#.#itemname#" = temp/>
				</cfif>
			</cfif>
			</cfloop>
		</cfif>
	...
	<cffunction name="filterWords" output="false" access="public" returntype="Any">
		...
		<cfset var result = StructNew()/>
		<cfset result.detected = true/>
		<cfset result.originalText = trim(REReplace(arguments.text,"(ï¿½|ï¿½)", "'", "ALL"))/>	<!---trim white space and deal with "smart quotes" from MS Word, etc.--->
